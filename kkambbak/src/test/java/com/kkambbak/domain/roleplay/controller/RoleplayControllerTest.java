...
public class RoleplayControllerTest extends KkambbakDocumentApiTester {
    @MockitoBean private RoleplayFacade roleplayFacade;
    @MockitoBean private RoleplayService roleplayService;

    @Test
    @DisplayName("roleplay scenarios 조회 API")
    void getAllRolePlayScenarios() throws Exception {
        //given
        List<RoleplayListResponseDto> mockList = List.of(
                new RoleplayListResponseDto(1L,"At a Cafe","카페에서 쓰이는 표현",6),
                new RoleplayListResponseDto(2L,"At school","학교 생활에서 쓰이는 표현",4),
                new RoleplayListResponseDto(3L,"At Hospital","병원에서 쓰이는 표현",10)
        );
    ...
    void startRoleplayScenario() throws Exception {
        ...
        var mockResponse = RoleplayDialoguesResponseDto.builder()
                ...
                .audioUrl("https://r2-storage/audio/1.wav")
                .coreWord("order")
                .build();
                ...
                .andDo(document("roleplay-start",
                        resource(ResourceSnippetParameters.builder()
                                ...
                                .responseFields(
                                        ...
                                        fieldWithPath("body.mismatchKorean").type(JsonFieldType.STRING).description("틀린 한국어 문장"),
                                        fieldWithPath("body.mismatchRomanized").type(JsonFieldType.STRING).description("틀린 로마자 표기"),
                                        fieldWithPath("body.mismatchEnglish").type(JsonFieldType.STRING).description("틀린 영문 문장"),
                                        fieldWithPath("body.audioUrl").type(JsonFieldType.STRING).description("TTS 오디오 URL"),
                                        fieldWithPath("body.coreWord").type(JsonFieldType.STRING).description("힌트 핵심 단어(영어)")
                                )
                                .build()
                        )
                ));
                ...
                .andDo(document("roleplay-evaluate",
                        resource(ResourceSnippetParameters.builder()
                                ...
                                .responseFields(
                                        ...
                                        fieldWithPath("status.description").type(JsonFieldType.STRING).optional().description("상태 설명"),
                                        fieldWithPath("body.dialogueId").type(JsonFieldType.NUMBER).description("문장 ID"),
                                        fieldWithPath("body.score").type(JsonFieldType.NUMBER).description("발음 점수 (0~100)"),
    ...
    void nextRoleplayDialogue() throws Exception {
        ...
        var mockResponse = RoleplayDialoguesResponseDto.builder()
                ...
                .mismatchEnglish("I'm sleepy.")
                .mismatchRomanized("naneun jollyeoyo")
                .audioUrl("https://r2-storage/audio/next_7.wav")
                .coreWord("Americano")
                .build();
        ...
        mockMvc.perform(post("/api/v1/roleplay/next?sessionId=1")
                ...
                .andDo(document("roleplay-next",
                        resource(ResourceSnippetParameters.builder()
                                ...
                                .description("""
                                    현재 세션의 다음 문장을 생성하고,
                                    한국어/영문/로마자 문장 및 TTS URL을 반환합니다.
                                    """)
                                .requestHeaders(
                                        headerWithName("Authorization").description("Bearer 토큰")
                                )
                ...
                .andDo(document("roleplay-complete",
                        resource(ResourceSnippetParameters.builder()
                                ...
                                .responseFields(
                                        ...
                                        fieldWithPath("body.totalSentence").description("전체 문장 수"),
                                        fieldWithPath("body.correctSentence").description("정답 문장 수"),
                                        fieldWithPath("body.completedAt").description("완료일 (yyyy-MM-dd)")
                                )
                                .build()
                        )
                ));
