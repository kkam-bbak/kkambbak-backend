...
import com.kkambbak.client.azure.service.TTSService;
import com.kkambbak.client.openai.dto.ChatMessage;
import com.kkambbak.client.r2.service.CloudflareR2AudioService;
...
import com.kkambbak.domain.roleplay.exception.*;
...
public class RoleplayFacade {
    ...
    private final CloudflareR2AudioService cloudflareR2AudioService;
     ...
     * 4. GPT가 응답한 문장 TTS 변환 및 로마나이즈 처리
    ...
    public RoleplayDialoguesResponseDto start(Long userId, Long scenarioId) {
        ...
        String audioUrl = generateTtsAndUpload(gptAnswer.getKorean());
        ...
        RoleplayDialogues roleplayDialogue = roleplayService.saveRoleplayDialogues(roleplaySession,gptAnswer,romanized,mismatchRomanized,1,SpeakerType.AI,audioUrl,gptAnswer.getCoreWord());
    ...
    public RoleplayDialoguesResponseDto next(Long userId,Long sessionId) {
        ...
        RoleplayDialogues roleplayDialogue = roleplayService.saveRoleplayDialogues(roleplaySession, gptAnswer, romanized,mismatchRomanized,nextIndex, nextSpeakerType,audioUrl,gptAnswer.getCoreWord());
    ...
    private String generateTtsAndUpload(String text) {
        byte[] bytes = ttsService.textToSpeech(text);
        String filename = UUID.randomUUID() + ".wav";
        try {
            return cloudflareR2AudioService.uploadAudio(bytes, filename);
        } catch (Exception e) {
            log.error("TTS upload fail: {}", filename, e);
            throw new TTSUploadFailException();
        }
    }
